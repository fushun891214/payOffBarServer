...
export const editGroup = async (req:Request,res:Response) => {
    try{
        const {groupID, members, groupName}: {
            groupID: string,
        ...
        const membersToAdd = members.filter(m => !existingMemberIDs.includes(m.userID));
        ...
        await Promise.all(
         ...
         const updatedMembers = await Promise.all(
            members.filter(m => existingMemberIDs.includes(m.userID))
                .map(async member => {
...
export const notifyUpdatePaymentStatus = async (req: Request,res: Response) => {
    try{
        ...
        const user = await User.findOne({userID:userID,fcmToken:{ $exists:true}});
